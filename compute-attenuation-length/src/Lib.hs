...
-- Instead of iterating [... | a <- A | b <- B] as nested loops, "iterate using
-- the same index"
{-# LANGUAGE ParallelListComp #-}

module Lib
    ( computeAttenuationLength
    ) where
...
u       = 1.66054e-27                   -- Mass of single amu in kg
...
a2m     = 1e-10                         -- Convert angstroms to meters
...
printMaterial mtrl = (matName mtrl) ++ "\t" ++ show (waveLength mtrl) ++ "\t" ++ show (density mtrl) ++"\t" ++  show (thickness mtrl)
...
--aveVel lam = hJs / (a2m * lam * u)

-- Convert between wavelength and velocity for a neutron from the other
aveVelLam :: Double -> Double
aveVelLam = (/) (hJs / (a2m * u))

-- Calculate the n*Sig for each element (J3:J5 in the spreadsheet)
-- Expects the wavelength (in AA)
nSigmai :: Double -> Double -> Element -> Double
nSigmai wvlngth numPerArea elmnt = 
    barn * numPerArea * (xsTotSctt elmnt + xsAbs elmnt)
    --barn * numPerArea * (xsTotSctt elmnt + (aveVelLam wvlngth) * (xsAbs elmnt))
...
computeAttenuationLength :: IO ()
computeAttenuationLength = do
    ...
    let material  = readInMaterial title props
        wvlngth   = waveLength material
        thcknss   = thickness material
        ...
        arealDensity = [ density material / molarMass * numAtoms e | e <- elements ]
        nPerAreaPrfctr = matFactor * thcknss  -- = atomsPerArea material molarMass
        ...
        numPerArea = [ matFactor * (numAtoms e) | e <- elements ]
        --numPerArea = [ nPerAreaPrfctr * (numAtoms e) | e <- elements ]
        nSigmas = [ nSigmai wvlngth n e | n <- numPerArea | e <- elements ]
        sigTotal = sum nSigmas
        attenuation = exp ((-sigTotal) * thcknss)
        phitsDensities = [ matFactor * barn * (numAtoms e) | e <- elements ]
        --phitsDensities = map (* (matFactor / 1e24)) [ numAtoms e | e <- elements ]
        arealDensities'= [ (elemName e, a) | e <- elements | a <- arealDensity ]
    ...
    putStrLn $ "\\Sigma_{total} = " ++ show sigTotal
    putStrLn $ "\nAreal densities:"
    mapM_ printTuple arealDensities'
    putStrLn $ "\nPHITS densities for " ++ title ++ ":"
    mapM_ printTuple phitsDensities'
    
    putStrLn ""
    putStrLn $ "Velocity        = " ++ show (aveVelLam wvlngth)
    putStrLn $ "Wavelength      = " ++ show wvlngth
    putStrLn $ "\\Sigma_{total} = " ++ show sigTotal
    putStrLn $ "Attenuation multiplicative factor from " ++ title ++ " at " ++ 
      show wvlngth ++ " \\AA: " ++ show attenuation
    putStrLn $ "Attenuation \"strength\" = 1/(above) = " ++ show (1 / attenuation)
